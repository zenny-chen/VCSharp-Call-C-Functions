...
namespace CSharpInvokeNativeCFunction
{
    class Program
    {
        [DllImport("libMySo.so", EntryPoint = "MyNativeCPrintLine", CharSet = CharSet.Ansi)]
        private static extern int MyNativeCPrintLine(string inputStr, int outputMaxLen, IntPtr outputStr, IntPtr pOutLength);

        
        [DllImport("libMySo.so", EntryPoint = "MyNativeModifyString", CharSet = CharSet.Unicode)]
        ...
        [DllImport("libMySo.so", EntryPoint = "RegisterCallbackMethods")]
        private static extern void RegisterCallbackMethods(ArrayFuncDelegate csharpMethod);


        [DllImport("libMySo.so", EntryPoint = "TestCSharpMethodCallback")]
